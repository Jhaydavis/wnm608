...
switch ($data->type) {
    ...
    case "products_all":
        $output['result'] = makeQuery(makeConn(), "SELECT * FROM `products` 
        ORDER BY `date_added` 
        DESC LIMIT 12");
        break;

    case "product_search":
        $output['result'] = makeQuery(makeConn(), "SELECT * FROM `products`  WHERE `name` LIKE '%$data->search%' OR
        `description` LIKE '%$data->search%' OR
        `type` LIKE '%$data->search%' 
            ORDER BY `date_added` 
            DESC LIMIT 12");
        break;
